// Facturas Bosque Conectado — v1.2.0
...
const LOGO_SRC = "ICONS/logo-full.png";
  ...
  return `
    ...
    <div class="fp-row2">
      <div class="fp-fecha">
        ...
        <div style="margin-top:6px;">${f.fecha}</div>
      </div>
...
function abrirPreview(f, id) {
  previewDiv.innerHTML = renderFacturaHTML(f);
  previewWrap.style.display = 'block';
  previewWrap.dataset.numero = f.numero;
  previewWrap.dataset.deleteId = id || '';
  document.getElementById('facturaPreviewOuter').scrollLeft = 0;
  document.getElementById('btnBorrarPreview').style.display = id ? 'inline-block' : 'none';
}
...
function descargarPdf() {
  ...
  const opt = {
    ...
    html2canvas: { scale: 2, backgroundColor: '#fdfcf7', scrollX: 0, scrollY: 0 },
...
document.getElementById('btnBorrarPreview').addEventListener('click', async () => {
  const id = previewWrap.dataset.deleteId;
  if (!id) return;
  if (!confirm('¿Borrar esta factura definitivamente? No se puede deshacer.')) return;
  try {
    await remove(ref(db, `${ROOT}/historial/${id}`));
...
async function cargarHistorial() {
  ...
  ids.forEach(id => {
    ...
    `;
    item.addEventListener('click', () => abrirPreview(f, id));
