...
document.getElementById('btnGuardarEmisor').addEventListener('click', async () => {
  ...
  await set(ref(db, `${ROOT}/emisor`), emisor);
...
function pintarSelectClientes() {
  ...
  sel.innerHTML = '<option value="">— Cliente puntual (sin guardar) —</option>';
...
function addLinea(datos) {
  ...
  tr.dataset.id = id;
  tr.innerHTML = `
    <td class="col-cant"><input type="number" class="in-cant" value="${datos?.cantidad ?? 1}" min="0" step="1"></td>
    <td><input type="text" class="in-desc" value="${datos?.descripcion ?? ''}" placeholder="Descripción"></td>
    <td class="col-precio"><input type="number" class="in-precio" value="${datos?.precio ?? ''}" min="0" step="0.01"></td>
    <td class="col-total"><span class="out-total">0,00 €</span></td>
    <td class="col-del"><button type="button" data-del="${id}">✕</button></td>
  `;
  lineasBody.appendChild(tr);
  tr.querySelectorAll('input').forEach(inp => inp.addEventListener('input', recalcular));
  tr.querySelector('[data-del]').addEventListener('click', () => { tr.remove(); recalcular(); });
  recalcular();
}
document.getElementById('btnAddLinea').addEventListener('click', () => addLinea());

function leerLineas() {
  return Array.from(lineasBody.querySelectorAll('tr')).map(tr => {
    const cantidad = parseFloat(tr.querySelector('.in-cant').value) || 0;
    const descripcion = tr.querySelector('.in-desc').value.trim();
    const precio = parseFloat(tr.querySelector('.in-precio').value) || 0;
...
function recalcular() {
  ...
  lineasBody.querySelectorAll('tr').forEach((tr, i) => {
    tr.querySelector('.out-total').textContent = eur(lineas[i].total);
  });
...
function descargarPdf() {
  const opt = {
    ...
    html2canvas: { scale: 2, backgroundColor: '#fdfcf7' },
